...
//-->
  ...
  function tab(id,hx,box,iClass,s,pr){
      ...
      function go_to(pr){
          for(var i=0;i<hxs.length;i++){
              if(!hxs[i].tmpClass){
                  hxs[i].tmpClass=hxs[i].className="on";
				  boxs[i].style.display="block";; // 展开状态：容器
                  //boxsClass[i].tmpClass=boxsClass[i].className+="on";
              }
              if(pr==i){
                  boxs[i].style.display="block";; // 展开状态：容器
				  boxs[i].id="this";
				  //boxsClass[npr].style.display="block";
				  var ulid=Pid("this","a");
				  ulid[nid2].style.color="#fff";
