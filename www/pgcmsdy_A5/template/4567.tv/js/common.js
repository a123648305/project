...
function ingetCookie(){
	...
	if( history_html==null){
		document.getElementById("histroydrop_con").innerHTML = "<div id=\"morelog\" class=\"histodo\">您的观看历史为空。<\/div>";
	}else{
		...
		if(tarray.length<1){
			...
			document.getElementById("histroydrop_con").innerHTML = "<ul id=\"playhistory\" class=\"highlight\">"+tarray+"</ul>";
		}
	}
};
...
jQuery(".closehis").click(function(){
 ...
 jQuery(".zk a").toggle(function(){
    jQuery("#player").css("width","488");
	jQuery("#cciframe").css("width","488");
	jQuery("#playlist").css("display","block");},
    function(){
    jQuery("#player").css("width","690");
	jQuery("#cciframe").css("width","690");
